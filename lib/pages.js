...
export function loadPage(slug) {
  try {
    ...
    try {
      const fallbackDir = path.join(ROOT, "content", "home-04");
      const meta = JSON.parse(fs.readFileSync(path.join(fallbackDir, "page.json"), "utf8"));
      return { meta, body: "" };
    } catch (err) {
      return { meta: { title: "Kazain Ventures", headAssets: [] }, body: "" };
    }
  }
}
